...
const renderComponent = () => {
    render(
        <RecoilRoot 
            initializeState={(snap) => snap.set(groupMembersState, ['영수', '영희'])}
            // initializeState={({set}) => set(groupMembersState, ['영수', '영희'])}
        >
            <ExpenseMain />
        </RecoilRoot>
    )
    ...
    // Error Message - description, amount, payer
...
describe('비용 정산 메인 페이지', () => {
    ...
    describe('새로운 비용이 입력', () => {
        ...
        const addNewExpense = async () => {
            ...
            await userEvent.type(dateInput, '2022-10-02')
            await userEvent.type(descInput, '장보기')
            await userEvent.type(amountInput, '30000')
            await userEvent.selectOptions(payerInput, '영수')
            await userEvent.click(addButton)
        ...
        test('날짜, 내용, 결제자, 금액 데이터가 정산 리스트에 추가된다', () => {
            ...
            const dateValue = within(expenseListComponent).getByText('2022-10-02')
